...
gr <- GRanges(seqnames = c("chrX"), strand = c("+", "-", "*"), ranges = IRanges(start = c(1,200, 1000), end = c(30, 299, 1200))) # mini genome

# let's see 
gr # telling us no seqlengths 
...
seqlevels(gr) <- c("chrX", "chrY") # use for comparison 
seqlevels(gr)
...
# findoverlap(subject,query)
ovf <- findOverlaps(peaks1, peaks2)
length(peaks1) # how many peaks 
length(peaks2)
length(ovf) # how many overlaps 
...
# a bunch or barely any is based on order of subject and querey 
```
...
table(table(crazy_ovf@from)) # find unique overlaps. WANT ALL!
...
crazy_ovf2 <-countOverlaps(crazy_peaks1, crazy_peaks2) # return a number you acutally want!!!
