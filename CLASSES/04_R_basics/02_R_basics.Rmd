...
CTCF_peak1 <- read_tsv(file.path(basepath, "/group/results/bwa/mergedLibrary/macs/broadPeak/CTCF_R2_peaks.broadPeak"))
...
CTCF_peak1 <- read_tsv(file.path(basepath, "/group/results/bwa/mergedLibrary/macs/broadPeak/CTCF_R2_peaks.broadPeak"), col_names=F)
...
names(CTCF_peak1) <- c('chromosome', 'start', 'end', 'name', 'score', 'strand', 
...
CTCF_peak1 <- CTCF_peak1[CTCF_peak1$chromosome %in% c(paste0("chr", 1:22), "chrM", "chrX", "chrY"),]
...
hist(CTCF_peak1$peak_width)

# looks like most are small, let's add more breaks

hist(CTCF_peak1$peak_width, breaks = 50)

# now let's set the x axis limits to ~3Kb

hist(CTCF_peak1$peak_width, breaks = 50, xlim = c(0,3000))
...
density <- density(CTCF_peak1$peak_width)
