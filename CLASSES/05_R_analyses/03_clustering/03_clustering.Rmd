...
# correlate 0 and 1 vector and that tells cluster
```
...
promoter_peak_occurence_matrix <- read.table("/scratch/Shares/rinnclass/CLASS_2023/erme3555/CLASS_2023/CLASSES/05_R_analyses/results/lncrna_mrna_promoter_peak_occurence_matrix.tsv")
...
# note the meta data
promoter_peak_occurence_matrix[1:2,19:24] # first 2 proteins (rows) 
# let's see if there are row and column names:

# row.names
row.names(promoter_peak_occurence_matrix)
# column names
colnames(promoter_peak_occurence_matrix) # all promoters all enseml id of what gene they are a promoter for 
...
test <- dist(promoter_peak_occurence_matrix[1:2,], method = "euclidean") # basically the same as pearson
test
...
test2 # .97 very similar in binding pattern 
...
peak_occurence_dist # get higher corr if your protein isnt bound to proomters and has a lot of 0's
# can correct using sparse
```
...
plot(bin_hier) # pol2 and tf it needs would both be bound 
# can see diustance on scale 

# let's take a look at what we got and see distance distribution
hist(bin_hier$height) # 1 is very far/different from 0 (the furthest branch is 1)
...
# although lcnrna less overlap with promoter tend to have similar binding profiles on lnc ad mrna
```
...
# seeing what's there :
XIST_promoter # only in XX to shut off one 
# How many DBPs bound
nrow(XIST_promoter)
